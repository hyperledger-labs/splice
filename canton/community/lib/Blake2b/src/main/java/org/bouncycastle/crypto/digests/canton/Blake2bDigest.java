package org.bouncycastle.crypto.digests.canton;
 ...
 * It's changed to support additional parameters that are required to implement Blake2xb on top of Blake2x.
 * Conversely, it disables other features (keying mechanism, personalization) not needed for Blake2xb
 */
    ...
    implements ExtendedDigest
{
    ...
    // Tree hashing parameters. While the tree mode is not supported, these are still used to implement the
    // Blake2xb XOF on top of Blake2b
   ...
   public Blake2bDigest(Blake2bDigest digest)
    {
        ...
        this.t1 = digest.t1;
        this.f0 = digest.f0;
        this.internalState = digest.internalState;
    ...
    public static byte[] digest(byte[] bytes, int digestBytes, int fanout, int depth, int leafLength, long nodeOffset, int nodeDepth, int innerHashLength) {
       byte[] out = new byte[digestBytes];
       Blake2bDigest d = new Blake2bDigest(digestBytes, fanout, depth, leafLength, nodeOffset, nodeDepth, innerHashLength);
       d.update(bytes, 0, bytes.length);
       d.doFinal(out, 0);
       return out;
    }

    public Blake2bDigest(int digestBytes, int fanout, int depth, int leafLength, long nodeOffset, int nodeDepth, int innerHashLength)
    {
        if (digestBytes < 0 || digestBytes > 64)
        {
            throw new IllegalArgumentException(
                "BLAKE2b digest byte length must be between 0 and 64 (inclusive)");
        }
    ...
    private void init()
    {
        if (chainValue == null)
        {
            ...
            chainValue[0] = blake2b_IV[0]
                ^ (digestLength | (keyLength << 8) | (fanout << 16) | (depth << 24) | (((long) leafLength) << 32));
