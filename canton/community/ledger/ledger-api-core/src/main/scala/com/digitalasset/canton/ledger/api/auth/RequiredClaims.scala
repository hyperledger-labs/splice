...
object RequiredClaims {
  ...
  def submissionClaims[Req](
    ...
    RequiredClaim.MatchApplicationId(applicationIdL) :: actAs.view
      .map(RequiredClaim.ActAs[Req])
      .toList ::: readAs.view.map(RequiredClaim.ReadAs[Req]).toList
  ...
  def eventFormatClaims[Req](eventFormat: EventFormat): List[RequiredClaim[Req]] =
    readAsForAllParties[Req](eventFormat.filtersByParty.keys) ::: eventFormat.filtersForAnyParty
      .map(_ => RequiredClaim.ReadAsAnyParty[Req]())
      .toList
