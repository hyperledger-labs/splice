...
class SegmentState(
  ...
  def discardedMessageCount: Int =
    discardedStaleViewMessagesCount + discardedRetransmittedCommitCertsCount + segmentBlocks.forgetNE
      .map(_.discardedMessages)
      .sum + viewChangeState.values.map(_.discardedMessages).sum
