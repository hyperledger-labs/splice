...
2. Parties using external signing for which the onboarding contracts
   have been setup through ``/v0/admin/external-party/setup-proposal``
...
2. Parties using external signing for which no corresponding
   ...
   ``/v0/admin/external-party/setup-proposal`` does setup such a
   ...
   a different partyt than the validator operator party. This also
...
In most cases, the preferred option of bypassing the limit is to setup
...
operator as this relies on the ``ValidatorRight`` contract.
