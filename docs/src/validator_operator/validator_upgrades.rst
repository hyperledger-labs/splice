...
* Please modify the file ``splice-node/examples/sv-helm/participant-values.yaml`` so that ``disableAutoInit`` is set to ``true``.
  This will ensure that the Canton participant does not generate new :ref:`identities <validator-upgrades-state>` for itself and instead remains ready to be initialized with the identities from the migration dump.
