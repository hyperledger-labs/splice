...
for cluster in $prod_clusters
  do
    if git show "main:cluster/deployment/${cluster}/config.yaml" | grep "releaseReference" | grep "${TARGET_BRANCH}"; then
        approve_job "${job_name}" "${cluster}"
    elif [ "${TARGET_BRANCH}" = "main" ]; then
        approve_job "${job_name}" "deployment"
    else
        echo "TARGET_BRANCH (${TARGET_BRANCH}) does not match releaseReference for ${cluster}"
    fi
  done
