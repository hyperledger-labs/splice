...
if [ "$APP_USER_PROFILE" = "on" ]; then
  ...
  grpc_health_probe -addr="localhost:2${CANTON_GRPC_HEALTHCHECK_PORT_SUFFIX}"
fi
if [ "$APP_PROVIDER_PROFILE" = "on" ]; then
  echo "Checking 3${CANTON_GRPC_HEALTHCHECK_PORT_SUFFIX}"
  grpc_health_probe -addr="localhost:3${CANTON_GRPC_HEALTHCHECK_PORT_SUFFIX}"
fi
if [ "$SV_PROFILE" = "on" ]; then
  echo "Checking 4${CANTON_GRPC_HEALTHCHECK_PORT_SUFFIX}"
  grpc_health_probe -addr="localhost:4${CANTON_GRPC_HEALTHCHECK_PORT_SUFFIX}"
fi
