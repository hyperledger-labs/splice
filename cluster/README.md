...
* a standalone validator `Validator1` along with wallet, ANS and splitwell UIs
...
| SVi Frontends                | Single Page Application | Auth for Wallet, ANS and SV UIs for SV1-4              |
| Splitwell UI                 | Single Page Application | Auth for Splitwell UI                                        |
| Validator1 UI                | Single Page Application | Auth for Wallet, ANS and Splitwell UIs for Validator1  |

### `canton-network-sv-test` Tenant

This tenant is used by components of the `sv-runbook` Pulumi stack to deploy:
* a supervalidator `SV` along with associated wallet, ANS and SV UIs
...
| ANS UI                 | Single Page Application | Auth for SV ANS UI                                     |
...
* a standalone validator `Validator` along with associated wallet and ANS UIs
...
| ANS UI                 | Single Page Application | Auth for Validator ANS UI                              |
...
10. Similar to 6, grant permissions also to the new CCI account:
   ...
   gcloud projects add-iam-policy-binding da-cn-shared \
      ...
      --role='roles/storage.objectViewer'
   ```
11. Grant permissions to the shared kms to the new CCI account:
...
12. Grant `Editor` permissions to the GKE node pool account

This is required for logging to work among other things.

13. Add service account da-gcr-image-pull@da-dev-gcp-it-security-project.iam.gserviceaccount.com with "Kubernetes Engine Viewer" permissions to the project
