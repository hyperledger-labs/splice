...
.. code-block:: bash
    ...
    curl -sSLf "https://wallet.validator.YOUR_HOSTNAME/api/validator/v0/admin/domain/data-snapshot" -H "authorization: Bearer <token>" -d '{"timestamp": "<timestamp>"}' -X GET -H "Content-Type: application/json" > dump.json
