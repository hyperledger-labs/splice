...
You can change the name of a running Super Validator (SV) when applying a network upgrade.
Before upgrading, make sure all other SVs have changed their configuration to approve your new SV name.

Renaming an SV entails the following steps:
1. As part of the upgrade, making sure the SV node uses the new SV name. This will create a new party ID for the SV node administrator.
2. Creating a temporary user in the participant with the "old" party ID from before the upgrade, which holds the coin that was held across the upgrade.
3. Logging into the temporary user and transferring its coins to the new SV node administrator.

**As part of the upgrade, making sure the SV node uses the new SV name. This will create a new party ID for the SV node administrator.**

Change the ``onboardingName`` and ``nodeName`` to a new name in ``sv-values.yaml`` and ``cometbft-values.yaml`` respectively.

Once your SV node is fully initialized, :ref:`login to your wallet UI <helm-sv-wallet-ui>` with the usual credentials of the ``validatorWalletUser``.
You can observe the new party ID on the top navigation bar, with a prefix that is your new SV name.
You will also notice the coin balance is lower than the value before upgrading. That is because renaming an SV will allocate a new ``Party Id`` and the coins are still owned by the old ``Party Id``.

Keep the browser window of the wallet UI open. We will transfer the coins from the old ``PartyId`` to the new one.

**Creating a temporary user in the participant with the "old" party ID from before the upgrade, which holds the coin that was held across the upgrade.**

In order to interact with the ledger API on the participant, you will need an access token.

If you are using an Auth0 setup, login to your Auth0 dashboard and make sure you have selected the correct tenant. Navigate to Applications > Applications on the left sidebar. Look for and click the Machine to Machine application named ``Validator app backend`` :ref:`you created previously <helm-sv-auth0>`.
Click "Quick Start". Under "Getting an access token for your API", copy and execute the command in the terminal to obtain the access token.

With the access token obtained above, please execute the following command in the terminal in order to allocate a party with the same ``Party Id`` you had before the rename.
Please modify the command as follows:

- Replace ``<your_hostname>`` with the hostname of your deployment.
- Replace ``<access_token>`` with the access token obtained above.
- Replace ``<original_sv_name>`` with the original sv name before renaming.

.. code-block:: bash

    curl --location 'https://directory.sv.svc.<your_hostname>/api/json-api/v1/parties/allocate' \
    --header 'Authorization: Bearer <access_token>' \
    --header 'Content-Type: application/json' \
    --data '{
        "identifierHint": "<original_sv_name>",
        "displayName": "<original_sv_name>"
    }'

Locate the ``identifier`` from the response, you will use that in the next step:
...
In your auth provider, create a temporary user account that will have permissions for the party ID you just created.
If you are using Auth0, that would be performed by navigating to User Management > User and clicking "Create User".
Locate the ``user_id`` from the new user page. It should be in a format of e.g. ``auth0|43b68e1e4978b000cefba352`` (or something similar for other auth providers).

Execute the following command in the terminal to create a new user in your participant associated with the allocated ``Party Id``.
Please modify the command as follows:

- Replace ``<your_hostname>`` with the hostname of your deployment.
- Replace ``<access_token>`` with the same access token you used before.
- Replace ``<user_id>`` with the user id of the temporary user.
- Replace ``<original_party_id>`` with the ``identifier`` obtained from the previous command.

.. code-block:: bash

    curl --location 'https://directory.sv.svc.<your_hostname>/api/json-api/v1/user/create' \
    --header 'Authorization: Bearer <access_token>' \
    --header 'Content-Type: application/json' \
    --data '{
        "userId": "<user_id>",
        "primaryParty": "<original_party_id>",
    "rights": [
            {
                "type": "CanActAs",
...
**Logging into the temporary user and transferring its coins to the new SV node administrator.**
...
Switch to the browser window of the wallet UI that logged in as ``validatorWalletUser``.
Locate the ``Party Id`` on the top navigation bar.  You should see the new SV name appear in that ``Party Id``.
Click the copy button to copy the ``Party Id``.

Switch to the browser window that logged in as the temporary user.
Navigate to Transfer. Paste the ``Party Id`` to ``Recipient``.

Determine the approximate transfer fee from the "Total Available Balance":

- For 0.0 CC to 1,000,000.0 CC, 1%
- For > 1,000,000.0 CC, 0.001%.

So if your available balance is 1,230,000.123 CC, the approximate transfer fee would be (1,000,000.0 * 0.01) + (230,000.123 * 0.00001) = 10,002.30000123 CC.

The transfer amount will be 1,230,000.123 CC (available balance) - 10,002.3 CC (approximate fee) = 1219997.823.

Please note that it is just an approximate calculation. For accurate transfer fee calculation please refer to :ref:`your Scan App UI <helm-scan-web-ui>`.

Enter the transfer amount and click the "SEND" button.

Switch to the browser window that logged in as ``validatorWalletUser``.
You will observe there is a new transfer offer pending to be accepted. Click accept to receive the fund.
