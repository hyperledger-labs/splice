...
.. code-block:: bash
    ...
    data=$(curl -sSLf `https://sv.sv.YOUR_HOSTNAME/api/sv/v0/admin/domain/data-snapshot` -H "authorization: Bearer <token>" -d '{"timestamp": "<timestamp>"}' -X GET -H "Content-Type: application/json")
