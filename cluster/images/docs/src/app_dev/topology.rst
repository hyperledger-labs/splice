...
.. TODO(M3-16): [polish] And link to a chapter that explains the standard components of CN, otherwise things like "app users have their own validator node" don't make sense to the reader.
...
.. TODO(M3-16): [polish] What does it mean for an app to be installed? What is the difference between "app" on the right side and "app Daml package" on the left side?
.. TODO(M3-16): [polish] Are "wallet" and "directory" not standard components of CN?
...
* from the diagram, we see that
  ...
  * there are five different APIs in use:
    ...
    3. A **Restricted Ledger API** is served by a validator node to the UI of an
       ...
       .. TODO(M3-16): [polish] Linked doc doesn't explain what an install contract is.

       .. TODO(M3-16): [unclear to me] Explain how does this work? Is this a regular ledger API with a new kind of JWT token? Or a completely new API?
       ...
       .. TODO(M3-16): explain where and how are these app access-tokens are used?
  ...
  * the app provider uses the ANS service to allocate a human-readable
    name to its on-ledger identity

  * the app user UI relies on the user having wallet, ANS, and app
    ...
    * the ANS app installation for looking up human-readable names for parties
