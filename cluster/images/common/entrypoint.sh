...
declare -a ARGS=( daemon --no-tty --log-encoder=json --log-level-stdout="${LOG_LEVEL_STDOUT:-DEBUG}" --log-level-canton="${LOG_LEVEL_CANTON:-DEBUG}" --log-level-root="${LOG_LEVEL_ROOT:-INFO}" --log-file-appender=off )
