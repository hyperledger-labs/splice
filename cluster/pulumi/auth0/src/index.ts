...
function newUiApp(
  ...
  urlPrefixes: string[],
  namespace: string,
  clusterBasenames: string[],
  auth0DomainProvider: auth0.Provider
): auth0.Client {
  const urls: string[] = urlPrefixes
    .map(prefix => {
      return clusterBasenames.map(clusterBasename => {
...
function cnAuth0(clusterBasenames: string[]) {
  ...
  const sv4UiApp = newUiApp(
    'sv4UiApp',
    'SV4 Frontends',
    'Used for the Wallet, CNS and SV UIs for SV4',
    ['wallet', 'cns', 'sv'],
    'sv-4.svc',
    clusterBasenames,
    provider
...
function svRunbookAuth0(clusterBasenames: string[]) {
...
function validatorRunbookAuth0(clusterBasenames: string[]) {
...
export function configureAuth0(clusterBasenames: string[]): pulumi.Output<Auth0ClusterConfig> {
  const cnAuth0Cfg = cnAuth0(clusterBasenames);
  const svRunbookAuth0Cfg = svRunbookAuth0(clusterBasenames);
  const validatorRunbookAuth0Cfg = validatorRunbookAuth0(clusterBasenames);
  return pulumi
...
export const clusterBasenames = JSON.parse(config.require('clusterBasenames')).sort();

export const auth0Cfg = configureAuth0(clusterBasenames);
