...
// FIXME: in many places we install the helm chart directly (search for splice-cluster-loopback-gateway), fix those to use this instead
...
export function installLoopback(
  ...
  const serviceEntry = new k8s.apiextensions.CustomResource('loopback-service-entry', {
  ...
  const virtualService = new k8s.apiextensions.CustomResource('loopback-virtual-service', {
    apiVersion: 'networking.istio.io/v1alpha3',
    kind: 'VirtualService',
    metadata: {
