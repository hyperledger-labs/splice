import { AcceptedGroupInvite, BalanceUpdate, Group as CodegenGroup } from "@daml.js/splitwise/lib/CN/Splitwise";
import { AcceptedAppPayment } from "@daml.js/wallet/lib/CN/Wallet";
import { Autocomplete, Box, Button, Divider, List, ListItem, Paper, Stack, Table, TableBody, TableCell, tableCellClasses, TableRow, TextField, Typography } from "@mui/material";
import { Empty } from "google-protobuf/google/protobuf/empty_pb";
import { useCallback, useState } from "react";
import { CompleteTransferRequest, CreateGroupInviteRequest, EnterPaymentRequest, GroupKey, InitiateTransferRequest, JoinGroupRequest, ListAcceptedAppPaymentsRequest, ListAcceptedGroupInvitesRequest, ListBalancesRequest, ListBalanceUpdatesRequest } from "./com/daml/network/splitwise/v0/splitwise_service_pb";
...
const key = (group: Contract<CodegenGroup>) =>
    new GroupKey().setId(group.payload.id.unpack).setOwnerPartyId(group.payload.owner).setProviderPartyId(group.payload.provider);
...
const Balances: React.FC<BalancesProps> = ({ directoryEntries, group, party }) => {
    ...
    const fetchBalances = useCallback(async () => {
        const balanceMap = (await splitwiseClient.listBalances(new ListBalancesRequest().setGroupKey(key(group)), null)).getBalancesMap();
...
const MembershipRequests: React.FC<MembershipRequestsProps> = ({ directoryEntries, group, provider }) => {
    const splitwiseClient = useSplitwiseClient();
    ...
    const fetchAcceptedInvites = useCallback(async () => {
        const invites = (await splitwiseClient.listAcceptedGroupInvites(new ListAcceptedGroupInvitesRequest().setGroupId(group.payload.id.unpack).setProviderPartyId(provider), null)).getAcceptedGroupInvitesList();
    ...
    const onAddMember = async (invite: Contract<AcceptedGroupInvite>) => {
        await splitwiseClient.joinGroup(new JoinGroupRequest().setAcceptedGroupInviteContractId(invite.contractId).setProviderPartyId(provider), null);
    };
    return <Box>
        <Stack sx={{ px: 2, py: 1 }}>
            ...
            <List>
                {acceptedInvites.map(invite =>
                    <ListItem key={invite.contractId}>
                        <Button onClick={() => onAddMember(invite)}>Add {directoryEntries.resolveParty(invite.payload.invitee)}</Button>
                    </ListItem>
                )}
            </List>
        </Stack>
...
const Entry: React.FC<EntryProps> = ({ directoryEntries, group, provider }) => {
    ...
    const onEnterPayment = async () => {
        await splitwiseClient.enterPayment(new EnterPaymentRequest().setDescription(paymentDescription).setQuantity(paymentQuantity).setGroupKey(key(group)).setProviderPartyId(provider), null);
    };
    ...
    const onInitiateTransfer = async () => {
        await splitwiseClient.initiateTransfer(new InitiateTransferRequest().setReceiverPartyId(transferReceiverEntry!.user).setQuantity(transferQuantity).setGroupKey(key(group)).setProviderPartyId(provider), null);
    };
    return <Stack>
        <Stack direction="row">
            <TextField label="Quantity" value={paymentQuantity} onChange={(event) => setPaymentQuantity(event.target.value)}></TextField>
            <TextField label="Description" value={paymentDescription} onChange={(event) => setPaymentDescription(event.target.value)}></TextField>
            <Button onClick={onEnterPayment}>Enter payment</Button>
        </Stack>
        <Stack direction="row" justifyContent="stretch">
            <TextField label="Quantity" value={transferQuantity} onChange={(event) => setTransferQuantity(event.target.value)}></TextField>
...
const BalanceUpdates: React.FC<BalanceUpdatesProps> = ({ directoryEntries, group }) => {
    ...
    const fetchBalanceUpdates = useCallback(async () => {
        const balanceUpdates = (await splitwiseClient.listBalanceUpdates(new ListBalanceUpdatesRequest().setGroupKey(key(group)), null)).getBalanceUpdatesList();
    ...
    const Update: React.FC<{ update: Contract<BalanceUpdate> }> = ({ update }) => {
        if (update.payload.update.tag === "ExternalPayment") {
            ...
            return <ListItem>
                {directoryEntries.resolveParty(value.payer)} payed {value.quantity} CC for {value.description}
            </ListItem>
        } else if (update.payload.update.tag === "Transfer") {
            const value = update.payload.update.value;
            return <ListItem>
                {directoryEntries.resolveParty(value.sender)} sent {value.quantity} CC to {directoryEntries.resolveParty(value.receiver)}
            </ListItem>;
        } else {
            return <ListItem>
                Netting: Not yet implemented
            </ListItem>;
        }
    };
    return <Stack sx={{ px: 2, py: 1 }}>
        ...
        <List>
            {balanceUpdates.map(c => <Update key={c.contractId} update={c} />)}
        </List>
    </Stack>;
};
...
const AcceptedAppPayments: React.FC<AcceptedAppPaymentsProps> = ({ directoryEntries, group, provider }) => {
    const splitwiseClient = useSplitwiseClient();
    ...
    const fetchAcceptedAppPayments = useCallback(async () => {
        const acceptedAppPayments = (await splitwiseClient.listAcceptedAppPayments(new ListAcceptedAppPaymentsRequest().setGroupKey(key(group)), null)).getAcceptedAppPaymentsList();
    ...
    const onRedeem = async (acceptedAppPayment: Contract<AcceptedAppPayment>) => {
        await splitwiseClient.completeTransfer(new CompleteTransferRequest().setGroupKey(key(group)).setAcceptedAppPaymentContractId(acceptedAppPayment.contractId).setProviderPartyId(provider), null);
    };

    const AcceptedPayment: React.FC<{ acceptedAppPayment: Contract<AcceptedAppPayment> }> = ({ acceptedAppPayment }) => {
        return <ListItem>
    ...
    return <Stack sx={{ px: 2, py: 1 }}>
        ...
        <List>
            {acceptedAppPayments.map((acceptedAppPayment) => <AcceptedPayment acceptedAppPayment={acceptedAppPayment} key={acceptedAppPayment.contractId} />)}
        </List>
    </Stack>
}
...
const Group: React.FC<GroupProps> = ({ directoryEntries, group, party, provider }) => {
    ...
    const onCreateInvite = async () => {
        await splitwiseClient.createGroupInvite(new CreateGroupInviteRequest().setObserverPartyIdsList(directoryEntries.getAllParties()).setGroupId(group.payload.id.unpack).setProviderPartyId(provider), null);
    };

    return <Paper elevation={3} sx={{ width: 600 }}>
        ...
        <Divider />
        {isOwner && <MembershipRequests group={group} directoryEntries={directoryEntries} provider={provider} />}
        ...
        <Divider />
        <Balances group={group} directoryEntries={directoryEntries} party={party} provider={provider} />
        <Divider />
...
const Groups: React.FC<GroupsProps> = ({ directoryEntries, party, provider }) => {
    ...
    return <Stack spacing={2}>
        {groups.map(group => <Group key={`${group.payload.owner}:${group.payload.id}`} directoryEntries={directoryEntries} group={group} party={party} provider={provider} />)}
    </Stack>;
};
