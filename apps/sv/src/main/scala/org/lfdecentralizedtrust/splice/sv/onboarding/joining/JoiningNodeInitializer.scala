    ...
    retryProvider.getValueWithRetries(
      ...
      for {
        ...
        if (mediatorTrafficState.extraTrafficLimit != unlimitedTraffic)
          throw Status.FAILED_PRECONDITION
            .withDescription(
              show"SV mediator $mediatorId does not have unlimited traffic on synchronizer $synchronizerId, expected ${unlimitedTraffic} but got ${mediatorTrafficState.extraTrafficPurchased}"
            )
            .asRuntimeException()
        ()
      },
