...
const GeneralInformationView: React.FC = () => {
  ...
  if (!resp.isLoading) {
    ...
    const dsoInfos = [
      createRow('dsoLeaderPartyId', data.dsoRules.payload.dsoDelegate.toString(), true),
      createRow('dsoPartyId', data.dsoPartyId, true),
      createRow('dsoEpoch', data.dsoRules.payload.epoch.toString()),
    ];
    return (
      <Box>
...
const DsoViewPrettyJSON = () => {
  ...
  if (dsoInfoResp.isLoading) {
    return <Loading />;
  }
  const dsoInfoData = dsoInfoResp.data;
