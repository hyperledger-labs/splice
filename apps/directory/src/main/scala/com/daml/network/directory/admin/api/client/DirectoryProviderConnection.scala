package com.daml.network.directory.admin.api.client
...
/** Connection to the admin API of the Directory.
  */
final class DirectoryConnection(
  ...
  override val serviceName = "directory"
  ...
  def getProviderPartyId()(implicit traceContext: TraceContext): Future[PartyId] = {
    ...
    prev match {
      ...
      case None =>
        for {
          partyId <- runCmd(GrpcDirectoryAppClient.GetProviderPartyId())
        } yield {
