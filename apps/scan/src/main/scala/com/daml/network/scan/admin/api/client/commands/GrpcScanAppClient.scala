    ...
    override def handleResponse(
      ...
      for {
        issuingMiningRounds <- response.issuingMiningRounds
          .traverse(round => Contract.fromProto(roundCodegen.IssuingMiningRound.COMPANION)(round))
          .leftMap(_.toString)
    ...
    override def handleResponse(
      ...
      for {
        coinRulesO <- response.coinRules
          .traverse(rules => Contract.fromProto(coinCodegen.CoinRules.COMPANION)(rules))
          .leftMap(_.toString)
