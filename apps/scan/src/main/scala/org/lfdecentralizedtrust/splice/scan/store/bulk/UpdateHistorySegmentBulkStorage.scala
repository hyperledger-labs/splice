...
class UpdateHistorySegmentBulkStorage(
      ...
      result <-
        if (
          ...
          if (updatesInSegment.nonEmpty) {
            ...
            logger.debug(s"Adding ${updatesInSegment.length} updates, between record time ${updatesInSegment.headOption.map(_.update.update.recordTime)} and ${updatesInSegment.lastOption.map(_.update.update.recordTime)}")
            ...
            logger.debug("No more updates inside the segment, done dumping updates from this segment")
  ...
  private def getSource(implicit
    ...
    Source
      ...
      .statefulMap(() => State(
        s3Connection.newAppendWriteObject(s"${segment.fromTimestamp}-${segment.toTimestamp}/updates_0.zstd"),
        0,
        0
      ))(
        {
          case (state, chunk) if state.s3ObjSize + chunk.bytes.length > config.bulkMaxFileSize =>
            logger.debug(s"Adding a chunk of ${chunk.bytes.length} bytes. The object size so far has been: ${state.s3ObjSize}, together they cross the threshold of ${config.bulkMaxFileSize}, so this is the last chunk for the object")
            logger.debug(s"First 4 bytes are: ${chunk.bytes.take(4).map(b => f"${b & 0xFF}%02X").mkString(" ")}")
            (
              State(
                s3Connection.newAppendWriteObject(s"${segment.fromTimestamp}-${segment.toTimestamp}/updates_${state.s3ObjIdx+1}.zstd"),
                state.s3ObjIdx + 1,
                0),
              ObjectChunk(chunk.bytes, state.obj, true, chunk.isLast, state.obj.prepareUploadNext())
            )
          case (state, chunk) =>
            logger.debug(s"Adding a chunk of ${chunk.bytes.length} bytes. The object size so far has been: ${state.s3ObjSize}, together they are not yet at the threshold of ${config.bulkMaxFileSize}")
            (
              State(
                state.obj,
                state.s3ObjIdx,
