...
class ScanHttpEncodingsTest extends StoreTestBase with TestEssentials with Matchers {
  ...
  "makeConsistentAcrossSvs on a nested Java Transaction" should {
    ...
    "honour the ApplyThreshold external-hash policy" in {
      val hash =
        HexString
          .parseToByteString("4d68f590e4a298d9617ebe07b98c6ecbe04b7f3d7a5327f0e0ad4719638302b7")
          .value
      val earlyRecordTime = Instant.parse("2025-01-01T00:00:00.000000Z")
      val lateRecordTime = Instant.parse("2025-06-01T00:00:00.000000Z")
      val threshold = Instant.parse("2025-03-01T00:00:00.000000Z")

      val events = Seq(mkCreatedEvent(0, "cid:only"))

      def outputHashWithPolicy(
          recordTime: Instant,
          policy: ExternalHashInclusionPolicy,
          thresholdOpt: Option[Instant],
      ): ByteString =
        ScanHttpEncodings
          .makeConsistentAcrossSvs(
            wrapTx(events, externalTransactionHash = hash, recordTime = recordTime),
            policy,
            thresholdOpt,
          )
          .update
          ...
          .asInstanceOf[TransactionTreeUpdate]
          .tree
          .getExternalTransactionHash

      // AlwaysInclude preserves the hash regardless of timing.
      outputHashWithPolicy(
        earlyRecordTime,
        ExternalHashInclusionPolicy.AlwaysInclude,
        None,
      ) shouldBe hash

      // ApplyThreshold with a later threshold strips the hash.
      outputHashWithPolicy(
        earlyRecordTime,
        ExternalHashInclusionPolicy.ApplyThreshold,
        Some(threshold),
      ) shouldBe ByteString.EMPTY

      // ApplyThreshold keeps the hash when the record time is at/after the threshold.
      outputHashWithPolicy(
        lateRecordTime,
        ExternalHashInclusionPolicy.ApplyThreshold,
        Some(threshold),
      ...
      // ApplyThreshold with no threshold configured strips the hash entirely.
      outputHashWithPolicy(
        lateRecordTime,
        ExternalHashInclusionPolicy.ApplyThreshold,
        None,
  ...
  "makeConsistentAcrossSvs on a ReassignmentEvent.Assign" should {
    "preserve the createdEvent's payload and metadata" in {
      val receiver = mkPartyId("receiver")
      val sourceDomain = SynchronizerId.tryFromString("dummy::source")
      val targetDomain = SynchronizerId.tryFromString("dummy::target")
      val amuletContract = amulet(receiver, 42.0, 13L, 2.0)

      val originalCreatedEvent = toCreatedEvent(amuletContract, Seq(dsoParty))
      val lapiAssignment = mkReassignment(
        offset = 98,
        event = ReassignmentEvent.Assign(
          unassignId = "unassignId",
          ...
          source = sourceDomain,
          target = targetDomain,
          createdEvent = originalCreatedEvent,
          counter = 71L,
        ),
        recordTime = CantonTimestamp.now(),
      )
      ...
      val normalised =
        ScanHttpEncodings
          ...
          .update
          .update
          .asInstanceOf[ReassignmentUpdate]
          .transfer
          .event
          .asInstanceOf[ReassignmentEvent.Assign]
          .createdEvent

      // The reassignment's createdEvent content is preserved.
      normalised shouldBe originalCreatedEvent
    }
  }
