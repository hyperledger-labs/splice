...
class AppActivityComputationTest extends AnyWordSpec with BaseTest {
  ...
  "AppActivityComputation" should {
    ...
    "return empty when there are no featured app confirmers" in {
      ...
      val result = withFeaturedAppParties(Set("partyB")).computeActivities(Seq((summary, verdict)))
    ...
    "return empty for rejected verdicts" in {
      ...
      val result = withFeaturedAppParties(Set("partyA")).computeActivities(Seq((summary, verdict)))
      result shouldBe empty
    }
    ...
    "split traffic equally among multiple app confirmers in a single envelope" in {
      ...
      val result =
        withFeaturedAppParties(Set("partyA", "partyB")).computeActivities(Seq((summary, verdict)))
      result should have size 1
    ...
    "produce one record per verdict in a batch" in {
      ...
      val result = withFeaturedAppParties(Set("partyA"))
        .computeActivities(Seq((summary1, verdict1), (summary2, verdict2)))
      result should have size 2
      result(0).recordTime shouldBe ts
      result(1).recordTime shouldBe ts2
    }
    ...
    "produce deterministic party ordering (alphabetical)" in {
      ...
      val result =
        withFeaturedAppParties(Set("zulu", "alpha", "mike")).computeActivities(
          Seq((summary, verdict))
        )
      result should have size 1
  ...
  "DvP settlement example" should {
    ...
    "Example 1: Bank1 is the only app provider" in {
      val result =
        withFeaturedAppParties(Set("Bank1")).computeActivities(Seq((dvpSummary, dvpVerdict)))
      result should have size 1
    ...
    "Example 2: Bank1 and Bank2 are app providers" in {
      val result =
        withFeaturedAppParties(Set("Bank1", "Bank2")).computeActivities(
          Seq((dvpSummary, dvpVerdict))
        )
      result should have size 1
    ...
    "Example 3: Alice, Bank1, and Bank2 are app providers" in {
      val result =
        withFeaturedAppParties(Set("Alice", "Bank1", "Bank2"))
          .computeActivities(Seq((dvpSummary, dvpVerdict)))
      result should have size 1
  ...
  private def withFeaturedAppParties(
      featured: Set[String]
  ): AppActivityComputation = new AppActivityComputation(loggerFactory) {
    override protected def getFeaturedAppProviders(recordTime: CantonTimestamp): Set[String] =
      featured
    override protected def getRoundNumber(recordTime: CantonTimestamp): Long = 0L
  }
