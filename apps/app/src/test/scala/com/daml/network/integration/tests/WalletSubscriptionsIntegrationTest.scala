...
import com.daml.network.codegen.java.splice.wallet.subscriptions.{Subscription, SubscriptionRequest}
...
class WalletSubscriptionsIntegrationTest
  ...
  "A wallet" should {
    "fail to get a non-existent subscription request" in { implicit env =>
      ...
      val errorString =
        s"HTTP 404 Not Found GET at '/api/validator/v0/wallet/subscription-requests/does%20not%20exist'. Command failed, message: contract id not found: ContractId(id = $nonExistentName"
