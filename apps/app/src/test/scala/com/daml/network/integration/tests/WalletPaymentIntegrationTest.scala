...
class WalletPaymentIntegrationTest
  ...
  "A wallet" should {
    "fail to get a non-existent payment request" in { implicit env =>
      ...
      val errorString =
        s"HTTP 404 Not Found GET at '/api/validator/v0/wallet/app-payment-requests/does%20not%20exist'. Command failed, message: contract id not found: ContractId(id = $nonExistentName"
