...
trait CnsFrontendTestUtil extends CNNodeTestCommon with CnsTestUtil {
  this: CommonCNNodeAppInstanceReferences & FrontendTestCommon =>

  private def allocateCnsEntry(
      cnsUiLogin: () => Unit,
      entryName: String,
  )(implicit
  ...
  def reserveCnsNameFor(
    ...
    clue(s"Reserving cns name: ${expectedName}") {
      val timeBeforeAllocate = LocalDateTime.now()
      allocateCnsEntry(cnsUiLogin, expectedName)
      ...
      // And then back to cns, where they are already logged in
      ...
      val goToCnsEntriesButton = eventually(timeUntilSuccess = 100.seconds) {
        find(id("cns-entries-button")).valueOrFail("The success page did not load.")
      }
      val timeAfterAllocate = LocalDateTime.now()

      click on goToCnsEntriesButton
