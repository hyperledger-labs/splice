...
import org.lfdecentralizedtrust.splice.util.{SpliceUtil, TimeTestUtil, TriggerTestUtil, WalletTestUtil}
  ...
  "automatically merge transfer inputs when the automation is triggered" in { implicit env =>
    ...
    TriggerTestUtil.setTriggersWithin(
      triggersToPauseAtStart =
        Seq(aliceValidatorBackend.userWalletAutomation(aliceWalletClient.config.ledgerApiUser).futureValue.trigger[CollectRewardsAndMergeAmuletsTrigger]),
    ) {
