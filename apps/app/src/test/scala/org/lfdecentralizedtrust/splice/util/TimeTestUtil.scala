...
trait TimeTestUtil extends TestCommon {
  ...
  /** This function advances time past the opening time of the next round and waits for the DSO round management automation for open, summarizing
    ...
    */
  def advanceRoundsToNextRoundOpening(implicit env: SpliceTestConsoleEnvironment): Unit =
    advanceRoundsToNextRoundOpening(synchronizeExternalPartyConfigStates = false)

  def advanceRoundsToNextRoundOpening(
      synchronizeExternalPartyConfigStates: Boolean = false
  )(implicit env: SpliceTestConsoleEnvironment): Unit = {
    if (synchronizeExternalPartyConfigStates) {
      clue("Tick duration for rounds and external party config states must match") {
        inside(sv1Backend.config.onboarding) { case Some(sv1Config: SvOnboardingConfig.FoundDso) =>
          sv1Config.initialTickDuration shouldBe sv1Config.initialTickDuration
        }
      }
    }
  ...
  def advanceTimeAndUpdateExternalPartyConfigStates(implicit env: SpliceTestConsoleEnvironment) = {
    ...
    val nextTargetArchive = configStates
  ...
  def waitForUpdateExternalPartyConfigStatesAutomation(implicit
    ...
    val configStates = sv1Backend.participantClientWithAdminToken.ledger_api_extensions.acs
      .filterJava(ExternalPartyConfigState.COMPANION)(dsoParty, _ => true)
    clue("Wait for UpdateExternalPartyConfigStateTrigger automation to quiet down") {
      eventually() {
        forAll(configStates) {
          _.data.targetArchiveAfter shouldBe >(now)
        }
      }
    }
  }
  ...
  def advanceTimeAndWaitForRoundAutomation(
    ...
    if (synchronizeExternalPartyConfigStates) {
      // First stop the triggers. This is required as otherwise after advancing time we may end up updating the external party config states
      // before the mining rounds so they still lag behind.
      activeSvs.foreach(
        _.dsoDelegateBasedAutomation
    ...
    if (synchronizeExternalPartyConfigStates) {
      actAndCheck(
        ...
        activeSvs.foreach(
          _.dsoDelegateBasedAutomation.trigger[UpdateExternalPartyConfigStateTrigger].resume()
        ),
      )(
        "ExternalPartyConfigState is updated",
        _ => {
          val configStates = sv1Backend.participantClientWithAdminToken.ledger_api_extensions.acs
            .filterJava(ExternalPartyConfigState.COMPANION)(dsoParty, _ => true)
          forAtLeast(1, configStates) { c =>
            c.data.holdingFeesOpenRoundNumber.number shouldBe lowestRound
          }
        },
      )
    }
  ...
  def advanceTimeAndWaitForExternalPartyConfigStateAutomation(
    ...
    actAndCheck("advancing time", advanceTime(duration))(
      ...
      _ => {
        ...
        val newConfigStates = sv1Backend.participantClientWithAdminToken.ledger_api_extensions.acs
          .filterJava(ExternalPartyConfigState.COMPANION)(dsoParty, _ => true)

        newConfigStates should not be configStates
      },
    )
  }
  ...
  def advanceTimeForRewardAutomationToRunForCurrentRound(implicit
    ...
    openRounds
      ...
      .foreach { opensAt =>
        ...
        advanceTime(Duration.between(now, advanceTo))
      }
  }
