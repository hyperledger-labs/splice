  ...
  override def environmentDefinition: SpliceEnvironmentDefinition =
    EnvironmentDefinition
      ...
      .withPreSetup { implicit env =>
        EnvironmentDefinition.simpleTopology4Svs(this.getClass.getSimpleName).preSetup(env)
        // Reset the set of vetted packages, otherwise the test may still the newer one even though the config contains the older versions.
        val vetted = sv1Backend.participantClient.topology.vetted_packages
          .list(
            store = Some(TopologyStoreId.Authorized),
            filterParticipant = sv1Backend.participantClient.id.filterString,
          )
        vetted match {
          case Seq(entry) if entry.item.packages.nonEmpty =>
            logger.info("Resetting SV1 vetted packages")
            val splicePackageIds: Set[String] =
              DarResources.packageResources.flatMap(_.all).map(_.packageId).toSet
            val filtered =
              entry.item.packages.filter(pkg => !splicePackageIds.contains(pkg.packageId: String))
            if (filtered != entry.item.packages) {
              ...
              logger.info(s"No need to reset SV1 vetted packages, current state: $vetted")
            }
          case _ =>
            logger.info(s"No need to reset SV1 vetted packages, current state: $vetted")
