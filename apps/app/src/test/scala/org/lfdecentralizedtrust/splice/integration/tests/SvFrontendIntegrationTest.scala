...
class SvFrontendIntegrationTest
  ...
  "SV UIs" should {
    ...
    "SV1 can create valid SRARC_CreateUnallocatedUnclaimedActivityRecord vote requests that can expire and get rejected by other SVs" in {
      implicit env =>
        val requestReasonUrl = "https://vote-request-url.com"
        val requestReasonBody = "This is a request reason."
        ...
        withFrontEnd("sv1") { implicit webDriver =>
          // If we try to create two vote requests for identical configs,
          // the second request will be rejected with "This vote request has already been created."
          def submitCreateUnallocatedUnclaimedActivityRecordRequestViaBackend(
              beneficiary: PartyId,
              amount: String,
              reason: String,
              expiresAt: Instant,
            ...
            val createUnallocatedUnclaimedActivityRecordAction: ActionRequiringConfirmation =
              new ARC_DsoRules(
            ...
            sv1Backend.createVoteRequest(
              sv1Backend.getDsoInfo().svParty.toProtoPrimitive,
              createUnallocatedUnclaimedActivityRecordAction,
              requestReasonUrl,
              requestReasonBody,
              if (expiresSoon) {
                new RelTime(java.time.Duration.ofMinutes(1).toMillis * 1000L)
          ...
          actAndCheck(
            "sv1 operator creates a new vote request with a short expiration time", {
              submitCreateUnallocatedUnclaimedActivityRecordRequestViaBackend(
                beneficiary = sv1Backend.getDsoInfo().svParty,
                amount = "1000",
                reason = "SV1 is doing great",
