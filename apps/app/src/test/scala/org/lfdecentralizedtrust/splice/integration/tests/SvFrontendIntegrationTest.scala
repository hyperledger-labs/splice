  ...
  "SV UIs" should {
    ...
    "can grant a validator license to an existing party" in { implicit env =>
      withFrontEnd("sv1") { implicit webDriver =>
        ...
        actAndCheck(
          "fill the party address field and click on the button to grant a validator license", {
            clue("fill party address") {
            ...
            clue("wait for the grant button to become enabled") {
              eventually() {
                find(id("grant-validator-license")).value.isEnabled shouldBe true
              }
            }

            clue("click the grant validator license button") {
              click on "grant-validator-license"
            }

            clue("click the confirmation button") {
