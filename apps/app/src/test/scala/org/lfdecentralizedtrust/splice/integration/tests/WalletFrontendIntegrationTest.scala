      ...
      "allow delegate to accept and reject minting delegation proposals and withdraw delegations" in {
        implicit env =>
          ...
          actAndCheck(
            ...
            _ => {
               aliceValidatorWalletClient.listMintingDelegationProposals().proposals should have size 3
            },
          )
          ...
          withFrontEnd("alice") { implicit webDriver =>
            ...
            actAndCheck(
              "Beneficiary 2 creates new minting proposal and UI refreshes", {
                createLimitedMintingDelegationProposal(beneficiary2Onboarding, validatorParty, expiresDayAfter, 18)
                webDriver.navigate().refresh()
              },
            )(
