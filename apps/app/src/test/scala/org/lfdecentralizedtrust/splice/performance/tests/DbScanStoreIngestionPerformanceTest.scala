...
class DbScanStoreIngestionPerformanceTest
    extends StoreTest
  ...
  "Ingestion performance test" in {
    ...
    storage
      .querySingle(
        sql"select count(*) from scan_acs_store".as[Int].headOption,
        "count",
      )
      .value
      .failOnShutdown("")
      .futureValue
      .valueOrFail("count is there") should be(13_747)

    storage
      .querySingle(
        sql"select count(*) from scan_txlog_store".as[Int].headOption,
        "count",
      )
      .value
      .failOnShutdown("")
      .futureValue
      .valueOrFail("count is there") should be(74_046)
  }
  ...
  def mkStore() = {
    ...
    new DbScanStore(
      ...
      _ => throw new RuntimeException("unsupported"),
      DomainMigrationInfo(
        domainMigrationId,
        None,
      ),
