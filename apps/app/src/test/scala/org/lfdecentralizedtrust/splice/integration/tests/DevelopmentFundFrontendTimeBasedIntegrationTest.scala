          ...
          withFrontEnd("alice") { implicit webDriver =>
            ...
            actAndCheck(
              "Alice withdraws the coupon via UI", {
                eventuallyClickOn(cssSelector("#active-coupons-table tbody tr button"))
    ...
    "reject allocation with invalid form inputs" in { implicit env =>
      ...
      withFrontEnd("alice") { implicit webDriver =>
        ...
        clue("Submit button is disabled with zero amount") {
          ...
          eventuallyClickOn(id("development-fund-allocation-amount"))
          textField("development-fund-allocation-amount").underlying.clear()
          textField("development-fund-allocation-amount").underlying.sendKeys("0")
          setDateTime("alice", "development-fund-allocation-expires-at", expiresAtFormatted)
          eventuallyClickOn(id("development-fund-allocation-reason"))
          textArea("development-fund-allocation-reason").underlying.sendKeys("Test reason")
        ...
        clue("Submit button is disabled without reason") {
          ...
          textField("development-fund-allocation-amount").underlying.clear()
          textField("development-fund-allocation-amount").underlying.sendKeys("10")
          textArea("development-fund-allocation-reason").underlying.clear()
        ...
        clue("Submit button is disabled when expires at is in the past") {
          ...
          textField("development-fund-allocation-amount").underlying.clear()
          textField("development-fund-allocation-amount").underlying.sendKeys("10")
          val pastExpiresAt = env.environment.clock.now.minusSeconds(3600)
          val pastExpiresAtFormatted = DateTimeFormatter
            .ofPattern("MM/dd/yyyy hh:mm a")
            .format(pastExpiresAt.toInstant.atZone(java.time.ZoneId.systemDefault()))
          setDateTime("alice", "development-fund-allocation-expires-at", pastExpiresAtFormatted)
          textArea("development-fund-allocation-reason").underlying.clear()
          textArea("development-fund-allocation-reason").underlying.sendKeys("Test reason")
        ...
        clue("Submit button is disabled with non-numeric amount") {
          ...
          eventuallyClickOn(id("development-fund-allocation-amount"))
          textField("development-fund-allocation-amount").underlying.clear()
          textField("development-fund-allocation-amount").underlying.sendKeys("abc")
          val expiresAt = env.environment.clock.now.plusSeconds(30 * 24 * 60 * 60)
          val expiresAtFormatted = DateTimeFormatter
            .ofPattern("MM/dd/yyyy hh:mm a")
            .format(expiresAt.toInstant.atZone(java.time.ZoneId.systemDefault()))
      ...
      clue("As user_2 (new DFM)") {
        withFrontEnd("bob") { implicit webDriver =>
          ...
          clue("Check: user_2's History List is empty") {
            eventually() {
              val historyRows = findAll(cssSelector("#coupon-history-table tbody tr")).toSeq
              historyRows.forall(row =>
                row.text.contains("No history events found")
              ) shouldBe true
            }
          }
