CREATE OR REPLACE FUNCTION
  functions.iso_timestamp(iso8601_string string)
  RETURNS timestamp AS (PARSE_TIMESTAMP('%FT%TZ', iso8601_string));

CREATE OR REPLACE FUNCTION functions.daml_prim_path(selector string)
    RETURNS string AS (
  CASE selector
...
CREATE OR REPLACE FUNCTION functions.daml_record_path(
  ...
  CONCAT('$',
         ...
         END,
         functions.daml_prim_path(prim_selector))
);

-- the most common JSON selection in this file
CREATE OR REPLACE FUNCTION functions.daml_record_numeric(
    daml_record json,
    path array<int64>
  ) RETURNS bignumeric AS (
  PARSE_BIGNUMERIC(JSON_VALUE(daml_record,
      functions.daml_record_path(path, 'numeric'))));

CREATE OR REPLACE FUNCTION functions.in_time_window(
...
CREATE OR REPLACE FUNCTION functions.sum_bignumeric_acs(
    path array<int64>,
    module_name string,
    entity_name string,
    as_of_record_time timestamp,
  ...
  SELECT
    COALESCE(SUM(PARSE_BIGNUMERIC(JSON_VALUE(c.create_arguments,
      functions.daml_record_path(path, 'numeric')))), 0)
  FROM
    devnet_da2_scan.scan_sv_1_update_history_creates c
  WHERE
    NOT EXISTS (
    SELECT
      TRUE
    FROM
      devnet_da2_scan.scan_sv_1_update_history_exercises e
    WHERE
    ...
    AND functions.in_time_window(as_of_record_time, migration_id,
          c.record_time, c.migration_id)));


-- Total unspent but locked Amulet amount.
CREATE OR REPLACE FUNCTION functions.locked(
    as_of_record_time timestamp,
    migration_id int64
  ) RETURNS bignumeric AS (
  functions.sum_bignumeric_acs(
...
CREATE OR REPLACE FUNCTION functions.unlocked(
    as_of_record_time timestamp,
    migration_id int64
  ) RETURNS bignumeric AS (
  functions.sum_bignumeric_acs(
...
CREATE OR REPLACE FUNCTION functions.unminted(
    as_of_record_time timestamp,
    migration_id int64
  ) RETURNS bignumeric AS (
  functions.sum_bignumeric_acs(
...
CREATE OR REPLACE FUNCTION functions.TransferSummary_minted(tr_json json)
    RETURNS bignumeric AS (
  functions.daml_record_numeric(tr_json, [0]) -- .inputAppRewardAmount
  + functions.daml_record_numeric(tr_json, [1]) -- .inputValidatorRewardAmount
  + functions.daml_record_numeric(tr_json, [2]) -- .inputSvRewardAmount
);

-- A choice's TransferSummary, if that choice is filtered by `minted`.
CREATE OR REPLACE FUNCTION functions.choice_result_TransferSummary(choice string, result json)
    RETURNS json AS (
  CASE choice
    WHEN 'AmuletRules_CreateExternalPartySetupProposal'
      -- .transferResult.summary
      THEN JSON_QUERY(result, functions.daml_record_path([3, 1], 'record'))
    WHEN 'AmuletRules_CreateTransferPreapproval'
      -- .transferResult.summary
      THEN JSON_QUERY(result, functions.daml_record_path([1, 1], 'record'))
    WHEN 'AmuletRules_BuyMemberTraffic'
      -- .summary
      THEN JSON_QUERY(result, functions.daml_record_path([1], 'record'))
...
CREATE OR REPLACE FUNCTION functions.minted(
    as_of_record_time timestamp,
    migration_id int64
  ) RETURNS bignumeric AS ((
  SELECT
    COALESCE(SUM(functions.TransferSummary_minted(
               functions.choice_result_TransferSummary(e.choice, e.result))),
             0)
  FROM
    devnet_da2_scan.scan_sv_1_update_history_exercises e
  WHERE
...
CREATE OR REPLACE FUNCTION functions.transferresult_fees(tr_json json)
    RETURNS bignumeric AS (
  -- .summary.holdingFees
  PARSE_BIGNUMERIC(JSON_VALUE(tr_json, functions.daml_record_path([1, 5], 'numeric')))
  -- .summary.senderChangeFee
  + PARSE_BIGNUMERIC(JSON_VALUE(tr_json, functions.daml_record_path([1, 7], 'numeric')))
  + (SELECT COALESCE(SUM(PARSE_BIGNUMERIC(JSON_VALUE(x, '$.numeric'))), 0)
     FROM
       UNNEST(JSON_QUERY_ARRAY(tr_json,
                -- .summary.outputFees
                functions.daml_record_path([1, 6], 'list'))) AS x));

CREATE OR REPLACE FUNCTION functions.result_burn(choice string, result json)
    RETURNS bignumeric AS (
  CASE choice
    WHEN 'AmuletRules_BuyMemberTraffic' THEN -- Coin Burnt for Purchasing Traffic on the Synchronizer
      ...
      PARSE_BIGNUMERIC(JSON_VALUE(result, functions.daml_record_path([2], 'numeric'))) -- .amuletPaid
      + PARSE_BIGNUMERIC(JSON_VALUE(result, functions.daml_record_path([1, 5], 'numeric'))) -- .summary.holdingFees
      + PARSE_BIGNUMERIC(JSON_VALUE(result, functions.daml_record_path([1, 7], 'numeric'))) -- .summary.senderChangeFee
    WHEN 'AmuletRules_Transfer' THEN -- Amulet Burnt in Amulet Transfers
      -- TransferResult
      functions.transferresult_fees(result)
    WHEN 'AmuletRules_CreateTransferPreapproval' THEN
      PARSE_BIGNUMERIC(JSON_VALUE(result, functions.daml_record_path([2], 'numeric'))) -- .amuletPaid
      + functions.transferresult_fees(JSON_QUERY(result, functions.daml_record_path([1], 'record'))) -- .transferResult
    WHEN 'AmuletRules_CreateExternalPartySetupProposal' THEN
      PARSE_BIGNUMERIC(JSON_VALUE(result, functions.daml_record_path([4], 'numeric'))) -- .amuletPaid
      + functions.transferresult_fees(JSON_QUERY(result, functions.daml_record_path([3], 'record'))) -- .transferResult
...
CREATE OR REPLACE FUNCTION functions.burned(
    as_of_record_time timestamp,
    migration_id_arg int64
  ) RETURNS bignumeric AS ((
  SELECT SUM(fees)
  FROM ((
            SELECT
                SUM(functions.result_burn(e.choice,
                                          e.result)) fees
            FROM
                devnet_da2_scan.scan_sv_1_update_history_exercises e
            WHERE
                ((e.choice IN ('AmuletRules_BuyMemberTraffic',
                    ...
                    OR (e.choice = 'TransferPreapproval_Renew'
                        AND e.template_id_entity_name = 'TransferPreapproval'))
              AND e.template_id_module_name = 'Splice.AmuletRules'
              AND functions.in_time_window(as_of_record_time, migration_id_arg,
                      e.record_time, e.migration_id))
        UNION ALL (-- Purchasing ANS Entries
            SELECT
                SUM(PARSE_BIGNUMERIC(JSON_VALUE(c.create_arguments, functions.daml_record_path([2, 0], 'numeric')))) fees -- .amount.initialAmount
            FROM
                devnet_da2_scan.scan_sv_1_update_history_exercises e,
                devnet_da2_scan.scan_sv_1_update_history_creates c
            WHERE
                ((e.choice = 'SubscriptionInitialPayment_Collect'
                    ...
                    AND c.contract_id = JSON_VALUE(e.result, functions.daml_record_path([2], 'contractId'))) -- .amulet
                    OR (e.choice = 'SubscriptionPayment_Collect'
                        AND e.template_id_entity_name = 'SubscriptionPayment'
                        AND c.contract_id = JSON_VALUE(e.result, functions.daml_record_path([1], 'contractId')))) -- .amulet
