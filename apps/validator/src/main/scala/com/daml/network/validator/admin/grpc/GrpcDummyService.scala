...
import com.daml.network.examples.v0.{DummyServiceGrpc, SomeDummyRequest, SomeDummyResponse}
...
class GrpcDummyService(protected val loggerFactory: NamedLoggerFactory)(implicit
    @nowarn("cat=unused")
    ec: ExecutionContext,
    tracer: Tracer,
) extends DummyServiceGrpc.DummyService
    with Spanning
    with NamedLogging {
