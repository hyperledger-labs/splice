...
abstract class StoreTest extends AsyncWordSpec with BaseTest {
  ...
  protected def additionalExpectedWarnings: Set[String] = Set.empty
  ...
  /** Runs the given Future, suppressing warnings generated by repeated database actions.
    ...
    */
  def withoutRepeatedIngestionWarning[A](f: => Future[A], maxCountRepeated: Int = 1): Future[A] = {
    ...
    loggerFactory.assertLogsSeq(SuppressionRule.Level(Level.WARN))(
      f,
      entries => {
        val (expectedRepeated, others) = entries.partition(isRepeatedIngestionWarning)
        val unexpected =
          others.filter(entry => !additionalExpectedWarnings.exists(entry.message.contains))
        expectedRepeated.length should be <= maxCountRepeated
