...
class CoinConfigScheduleTest extends AnyWordSpec with BaseTest {
  ...
  private def mkConfig(maxNumInputs: Int): cc.coinconfig.CoinConfig[cc.coinconfig.USD] =
    CNNodeUtil.defaultCoinConfig(
      dummyTickDuration,
      maxNumInputs,
      dummyDomainId,
    )

  private val schedule1 = CoinConfigSchedule(mkConfig(0), Seq.empty)
  private val schedule2 = CoinConfigSchedule(
    mkConfig(0),
    Seq(
  ...
  private def testCase(schedule: CoinConfigSchedule, asOf: Int, expectedMaxNumInputs: Int) = {
  ...
  "CoinConfigSchedule.getConfigAsOf" should {
