...
abstract class MultiDomainExpiredContractDsoDelegateTrigger[
    C,
    TCid <: ContractId[_],
    T,
](
  ...
  override final protected def listReadyTasks(now: CantonTimestamp, limit: Int)(implicit
    ...
    listExpiredContracts(now, PageLimit.tryCreate(context.config.parallelism * 10))(tc).map(seq =>
      Random.shuffle(seq)
    )
...
object MultiDomainExpiredContractDsoDelegateTrigger {
  type Template[TCid <: ContractId[_], T] =
    MultiDomainExpiredContractDsoDelegateTrigger[Contract.Companion.Template[TCid, T], TCid, T]
