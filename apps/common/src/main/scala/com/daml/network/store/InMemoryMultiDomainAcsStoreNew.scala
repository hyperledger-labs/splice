...
class InMemoryMultiDomainAcsStoreNew[TXE <: TxLogStoreNew.Entry](
  ...
  import InMemoryMultiDomainAcsStoreNew.reassignmentContractOrder
  ...
  private var stateVar: InMemoryMultiDomainAcsStoreNew.State[TXE] =
    InMemoryMultiDomainAcsStoreNew.State(
      offset = None,
  ...
  private def updateState[T](
      f: InMemoryMultiDomainAcsStoreNew.State[TXE] => (
          InMemoryMultiDomainAcsStoreNew.State[TXE],
          T,
      )
  ): Future[T] = {
    Future {
    ...
    override def ingestUpdate(
      ...
      update match {
        case TransactionTreeUpdate(tree, treeProto) =>
          updateState(
            _.ingestTransaction(domain, tree, treeProto, contractFilter, txLogConfig.parser, logger)
          ).map { case (summary, offsetChanged, offsetIngestionsToSignal) =>
  ...
  def getLatestTxLogEntry(query: (TXE) => Boolean = (_: TXE) => true)(implicit
      ec: ExecutionContext
  ): Future[TXE] =
    Future {
      stateVar.txLog.view
  ...
  def collectLatestTxLogEntryWithOffset[T](query: PartialFunction[TXE, T])(implicit
      ec: ExecutionContext
...
object InMemoryMultiDomainAcsStoreNew {
  ...
  /** General assumption: We're connected to both source and target domain.
    ...
    */
  private case class State[TXE <: TxLogStoreNew.Entry](
    ...
    private def removeOffsetSignalsBefore(
        offset: String
    ): (State[TXE], Map[String, Promise[Unit]]) = {
    ...
    def addOffsetToSignal(
        offset: String
    ): (State[TXE], Future[Unit]) = {
      offsetIngestionsToSignal.get(offset) match {
        case None =>
          val p = Promise[Unit]()
          val newSt: State[TXE] = copy(
