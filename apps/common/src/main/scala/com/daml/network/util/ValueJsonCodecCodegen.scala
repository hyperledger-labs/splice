...
object ValueJsonCodecCodegen {
  ...
  /** In the ledger API, the result of a choice is represented as a JavaApi.Value.
    ...
    * to a JavaApi.Value
    */
  private def choiceResultToValue(result: Any): JavaApi.Value = result match {
    // If the result type is a named record
    case ddt: DefinedDataType[?] => ddt.toValue

    // If the result type is Unit
    case v: JavaApi.Value => v
    ...
    case _ => throw new RuntimeException(s"Unsupported choice result type: ${result.getClass}")
  }
