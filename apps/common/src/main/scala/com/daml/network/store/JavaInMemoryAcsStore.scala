...
/** In-memory implementation of an [[AcsStore]] intended to be embedded in the
  ...
  */
class JavaInMemoryAcsStore(
    override protected val loggerFactory: NamedLoggerFactory,
    override val contractFilter: JavaAcsStore.ContractFilter,

    // Boolean flag to enable very verbose state update logging
    logAllStateUpdates: Boolean = false,
)(implicit
    ec: ExecutionContext
) extends JavaAcsStore
    with NamedLogging {
  ...
  private var stateVar: JavaInMemoryAcsStore.State =
    JavaInMemoryAcsStore.State(
      None,
      0,
  ...
  private def updateState[T](
      f: JavaInMemoryAcsStore.State => (JavaInMemoryAcsStore.State, T)
  ...
  val ingestionSink: JavaAcsStore.IngestionSink = new JavaAcsStore.IngestionSink with NoTracing {
  ...
  private def offsetAndStateAfterIngestingAcs(): Future[(String, JavaInMemoryAcsStore.State)] =
