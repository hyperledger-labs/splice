...
-- Note: the only operation that we need to support efficiently is streaming updates for a given history between two given offsets
create index updt_hist_tran_hi_mi_rt on update_history_transactions (history_id, migration_id, participant_offset);
...
(
    ...
    -- Update payload
    -- TODO(#10487): there's probably some columns missing and it might make sense to split this
    -- into two tables: one for assignments and one for unassignments
    reassignment_counter        int not null,
    target_domain               text,
    source_domain               text,
    submitter                   text,
...
-- Note: the only operation that we need to support efficiently is streaming updates for a given history between two given offsets
create index updt_hist_reas_ni_di_rt on update_history_reassignments (history_id, migration_id, participant_offset);
...
create unique index updt_hist_reas_unique on update_history_reassignments (history_id, migration_id, participant_offset);
